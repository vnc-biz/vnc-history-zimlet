...
biz_vnc_lightweight_history_HandlerObject.prototype.doDrop = function(droppedItem) {
	...
	this.animationDialog = new ZmDialog({
		view: this.animation,
	...
	if (droppedItem instanceof Array) {
		...
		if (obj.type == "CONV") {
			...
			myMsg.load({
				"callback": new AjxCallback(this, this._loadCallBack, [myMsg])
			});
		} else if (obj.type == "MSG") {
			obj.load({
				"callback": new AjxCallback(this, this._loadCallBack, [obj])
			});
		}
	}
};
...
biz_vnc_lightweight_history_HandlerObject.prototype._loadCallBack = function(response) {
	...
	this.pbDialog = new ZmDialog({
		title: biz_vnc_lightweight_history.mailHistory,
		view: this.pView,
		parent: this.getShell(),
	...
	this.pbDialog.setButtonListener(
		...
		new AjxListener(
			this,
			this._printBtnListener,
			[fromAddr.address, sender, subject, messageID]
		)
	);
	this.pbDialog.setButtonListener(
...
biz_vnc_lightweight_history_HandlerObject.prototype._createDialogView = function(messageID, fTree, s, from, sender) {
	...
	html[i++] = "<td>" + biz_vnc_lightweight_history.dateLabel + "</td><td>" +biz_vnc_lightweight_history.receiver +
			"</td><td>" + biz_vnc_lightweight_history.eventLabel + "</td><td>" + biz_vnc_lightweight_history.moveLableto + "</td>";
	html[i++] = "</tr>";
	if (from != sender) {
		html[i++] = "<tr>";
...
biz_vnc_lightweight_history_HandlerObject.prototype._printBtnListener = function(from, sender, subject, messageID) {
	jspUrl = this.getResource(
		"lightweightmailprint.jsp?s=" + subject + "&from=" + from + "&mainsender=" + sender +
		"&msgid=" + messageID + "&locale=" + appCtxt.getSettings().get(ZmSetting.LOCALE_NAME)
	);
	window.open(
		jspUrl,
		"",
		"menubar=yes,location=no,resizable=yes,scrollbars=yes,status=yes,width=500,height=500"
	);
}
